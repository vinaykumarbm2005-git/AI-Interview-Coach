...
interface TechnicalRoundPageProps {
  onBackToDashboard: () => void;
}

export const TechnicalRoundPage: React.FC<TechnicalRoundPageProps> = ({ onBackToDashboard }) => {
  const { selectedDomain, submitRound, saveAnswerLocally, savedAnswers, setRoundStatus } = useInterview();
  const [questions, setQuestions] = useState<Question[]>([]);
  const [currentIndex, setCurrentIndex] = useState(0);
  const [answers, setAnswers] = useState<Record<number, string>>(() => savedAnswers['technical'] || {});
  ...
  useEffect(() => {
    ...
    async function load() {
      setIsLoading(true);
      setRoundStatus('technical', 'In Progress');
      const qList = await fetchAIQuestions(selectedDomain, 'technical', 15);
      if (isMounted) {
        setQuestions(qList);
        setIsLoading(false);
      }
  ...
  const handleAnswerChange = (val: string) => {
    ...
    saveAnswerLocally('technical', qId, val);
  };
  ...
  const handleSubmit = async () => {
    ...
    await submitRound('technical', questions, answers);
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="text-center space-y-3">
          ...
          <p className="text-sm font-bold text-slate-800">Generating Dynamic AI Questions for {selectedDomain}...</p>
          <p className="text-xs text-slate-500">Tailoring 15 technical questions based on your domain context</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
      ...
      {showThankYou && <ThankYouOverlay roundName="Technical Round" />}
        ...
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-bold text-slate-900">Technical Round</h2>
      ...
      <main className="max-w-7xl mx-auto w-full px-6 py-6 flex-1 grid grid-cols-1 md:grid-cols-2 gap-6 items-stretch">
        ...
        <div className="h-full flex flex-col justify-between space-y-4">
          <AnswerTextArea
            ...
            placeholder="Type your technical response here. Auto-saves locally to prevent data loss on refresh..."
          />
          ...
          <div className="bg-white border border-slate-200/80 shadow-soft rounded-2xl p-4 flex items-center justify-between">
            ...
            {currentIndex === questions.length - 1 ? (
              ...
              >
                <span>Submit Technical Round</span>
